...
async def add_completed_games(ctx, *games):
    ...
    if games:
        ...
        db_sess.commit()
        await ctx.send(f"{ctx.author.name} добавляет себе в пройденые игры: {gm}")
    else:
...
async def add_favorite_games(ctx, *games):
    ...
    if games:
        ...
        db_sess.commit()
        await ctx.send(f"{ctx.author.name} добавляет себе в избранные игры: {gm}")
    else:
        await ctx.send("Что-то пошло не так")
...
async def search(ctx, *name):
    ...
    if user:
        if user.name == str(nm) and user.favorite_games or user.completed_games:
            ...
            if user.completed_games:
                await ctx.send(f"Завершенные игры: {user.completed_games}")
        else:
            await ctx.send("Веденно некоректное имя или данный пользователь не заригестрирован.")
    else:
        await ctx.send("Веденно некоректное имя или данный пользователь не заригестрирован.")
...
async def add_video_card(ctx, *video_card):
    ...
    if vc:
        ...
        db_sess.commit()
        await ctx.send(f"{vc} сохранено и будет использоватся для поиска и создания записей.")
    else:
        await ctx.send("Что-то пошло не так")
...
async def add_ram(ctx, *ram):
    ...
    if 1 <= ram <= 128:
        ...
        user.ram = ram
        db_sess.commit()
        await ctx.send(f"{ram}GB сохранено и будет использоватся для поиска и создания записей.")
    else:
        await ctx.send("Что-то пошло не так")
...
async def help(ctx):
    ...
    emb.add_field(name='search', value="Поиск по имени дискорд")
    emb.add_field(name='add_video_card', value="Дабавлени видеокарты")
    emb.add_field(name='add_cpu', value="Дабавлени процессора")
    emb.add_field(name='add_ram', value="Дабавлени оперативной памяти")
    ...
    emb.add_field(name='sk', value="Сменить радио")
...
async def on_message(message):
    ...
    if message.content.lower() in ["во что бы поиграть", "подходящее игры"]:
        gems = {}
        db_sess = db_session.create_session()
    ...
    if message.content.lower()[:3] == "бот":
        await message.channel.send(random.choice(["Я не магу понять ваше сообщение",
